...
import type { InpaintingModelChoice } from '@koharu/bridge/protocol'
...
const providerName = (provider: string) => providerNames[provider] ?? provider
...
export function ImageModelPicker({
  ...
  loading?: boolean
  disabled?: boolean
  onSelect: (choice: InpaintingModelChoice) => void
}) {
  ...
  const results = useMemo(
    () =>
      models.filter((model) => {
        ...
        return [model.name, model.model, providerName(model.provider)].some((candidate) =>
  ...
  return (
    <div className='min-w-0 overflow-hidden'>
      <div className='mb-1 flex h-7 items-center border-b border-border/60 px-0.5 pb-1'>
        <span className='text-[11px] font-medium'>{t('modelPicker.title')}</span>
      </div>
        ...
        <div className='grid min-w-0 gap-0.5 py-0.5'>
          {results.map((model) => {
            ...
            const selected = value
              ? value.provider === model.provider && value.model === model.model
              : false
            return (
              <Button
                key={key}
